...
function lowerCaseStrings(strings) {
  const lowercase = [];
  strings.forEach(element => lowercase.push(element.toLowerCase()));
  return lowercase;
}
...
function removeApple(strings) {
  const appleless = strings.filter(item => item !== 'apple');
  return appleless;
}
...
function getFullNames(runners) {
  const fullNames = runners.map(item => `${item.last_name}, ${item.first_name}`);
  return fullNames;
}
...
function firstNamesAllCaps(runners) {
  const capitalized = runners.map(item => item.first_name.toUpperCase());
  return capitalized;
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  const runnersByShirtSize = runners.filter(element => element.shirt_size === tShirtSize);
